...
public class PersianCalendar extends GregorianCalendar {
  ...
  public void setPersianDate(int persianYear, int persianMonth, int persianDay) {
    ...
    setTimeInMillis(convertToMilis(PersianCalendarUtils.persianToJulian(
        this.persianYear > 0 ? this.persianYear : this.persianYear + 1, this.persianMonth - 1,
        this.persianDay)));
  }
  ...
  public String getPersianLongDateAndTime() {
    return getPersianLongDate() + " ساعت " + get(HOUR_OF_DAY) + ":" + get(MINUTE) + ":" + get(
        SECOND);
  }
  ...
  public String getPersianShortDate() {
    // calculatePersianDate();
    return "" + formatToMilitary(this.persianYear) + delimiter + formatToMilitary(
        getPersianMonth() + 1) + delimiter + formatToMilitary(this.persianDay);
  }

  public String getPersianShortDateTime() {
    return ""
        + formatToMilitary(this.persianYear)
        + delimiter
        + formatToMilitary(getPersianMonth() + 1)
        + delimiter
  ...
  public void addPersianDate(int field, int amount) {
    ...
    if (field == YEAR) {
      ...
      setPersianDate(this.persianYear + ((getPersianMonth() + 1 + amount) / 12),
          (getPersianMonth() + 1 + amount) % 12, this.persianDay);
      return;
    }
    add(field, amount);
    calculatePersianDate();
  ...
  @Override public void set(int field, int value) {
